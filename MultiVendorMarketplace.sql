--all codes are for sql server management studio 21
...
Weak Entity:it depends on other entity
OrderItem (OrderItem depends on Orders,If there is no Order, there cannot be any OrderItem.)
Associative Entities(An associative entity is used to resolve Many-to-Many (M:N) relationships)
In our project:ProductCategory and WarehouseStock are associative entity 
ProductCategory : One Product can belong to many Categories,One Category can have many Products
WarehouseStock: One Warehouse stores many Variations,One Variation can exist in many Warehouses
...
SELECT *
FROM Orders
WHERE OrderDate >= DATEADD(DAY,-30,GETDATE());-- getdate means today,-30 means 30 days before from today
...
SELECT DISTINCT C.FullName --distinct means  one name show one time
FROM Customer C
